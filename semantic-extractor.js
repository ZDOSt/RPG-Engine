...
const COMPACT_LEDGER_CONTRACT = [
    ...
    '- RelationshipEngine[index].timeLapseExplicit is strict and unambiguous. It is a per-turn transition flag, not a general scene-state flag. Return Y only if the latest {{user}} input itself establishes that time has advanced from the immediately previous scene/assistant response into a later time: next morning, next day, after waking, after sleep/rest has already completed, after travel/downtime/separation, or another explicit time skip. Return N if the scene is already after a prior time skip but the latest {{user}} input only continues that same scene, waits a few minutes, talks, acts, looks around, or otherwise remains in the same outing/day beat. Return N if {{user}} only lies down, goes to sleep, prepares to rest, intends to rest, says they will return later, or otherwise describes an action before time has actually passed. Return N if the text only describes future-tense plans, promises, intentions, brief pauses, momentary silence, same-scene continuation, vague later wording, or merely saying they will meet later.',
