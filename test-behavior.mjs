...
const tests = [
  ...
  },
  {
    ...
    run() {
      ...
      assert.equal(delta.npcs[0].condition, 'bruised');
      assert.deepEqual(delta.npcs[0].woundsAdd, []);
      assert.deepEqual(delta.npcs[0].statusAdd, []);
    },
  },
  {
  ...
  },
  {
    ...
    run() {
      ...
      assert.match(prompt(report), /choose the concrete wound and affected body area from the NPC attack context/i);
      assert.match(auditPrompt(report), /detailMode:narrator_contextual/i);
      assert.equal(report.trackerUpdate.npcs.Seraphina.condition !== 'healthy', true);
  ...
  {
    name: '08 active enemy initializes at H4',
    run() {
      const report = runCase({
      ...
      assert.equal(report.trackerUpdate.npcs.Bandit.currentDisposition.H, 4);
    },
  },
  {
  ...
  {
    name: '09a stored negative history initializes H3 despite neutral semantic relationship',
    run() {
      const tracker = {
        Val: trackerEntry({
          currentDisposition: null,
          userHistory: { knowsUser: 'Y', standing: 'negative' },
        }),
      };
      const report = runCase({
      ...
      assert.equal(auditIncludes(report, 'initPreset=knownNegative'), true);
    },
  },
  {
    name: '09b stored positive history initializes B3',
    run() {
      const tracker = {
        Mira: trackerEntry({
          currentDisposition: null,
          userHistory: { knowsUser: 'Y', standing: 'positive' },
        }),
      };
      const report = runCase({
      ...
      assert.deepEqual(report.trackerUpdate.npcs.Mira.currentDisposition, { B: 3, F: 2, H: 2 });
      assert.equal(auditIncludes(report, 'initPreset=knownPositive'), true);
    },
  },
  {
    name: '09c fear-relevant user race deterministically raises fear only without peer immunity',
    run() {
      const tracker = {
        Villager: trackerEntry({ currentDisposition: null }),
        DemonPeer: trackerEntry({
          currentDisposition: null,
          raceProfile: { race: 'demon', category: 'demonic', fearProfile: 'peer' },
        }),
      };
      const persona = 'Race: Demon\nPHY: 6\nMND: 6\nCHA: 6';
      const report = runCase({
        ...
        ledger: baseLedger({
          ...
          relationshipEngine: [relationship('Villager'), relationship('DemonPeer')],
        }),
      });
      assert.equal(report.trackerUpdate.npcs.Villager.currentDisposition.F, 3);
      assert.equal(report.trackerUpdate.npcs.DemonPeer.currentDisposition.F, 2);
    },
  },
  {
    name: '09d explicit active character card hatred seeds deterministic negative history',
    run() {
      const tracker = {
        Val: trackerEntry({ currentDisposition: null }),
      };
      const report = runCase({
        userText: 'I approach Val and say hello.',
        tracker,
        cardFields: {
          name: 'Val',
          description: 'Val hates {{user}} and distrusts every word they say.',
        },
        ledger: baseLedger({
          resolutionEngine: {
            identifyGoal: 'Talk',
      ...
      assert.deepEqual(report.trackerUpdate.npcs.Val.currentDisposition, { B: 1, F: 2, H: 3 });
      assert.equal(report.trackerUpdate.npcs.Val.userHistory.standing, 'negative');
    },
  },
  {
    name: '09d.1 macro-expanded persona name hatred seeds deterministic negative history',
    run() {
      const tracker = {
        Val: trackerEntry({ currentDisposition: null }),
      };
      const report = runCase({
        ...
        cardFields: {
          name: 'Val',
          name1: 'Aelemar',
          description: 'This character HATES Aelemar.',
        },
        ledger: baseLedger({
          resolutionEngine: {
            identifyGoal: 'Talk',
  ...
  {
    name: '09d.2 structured relationship to persona name seeds deterministic negative history',
    run() {
      const tracker = {
        Val: trackerEntry({ currentDisposition: null }),
      };
      const report = runCase({
        ...
        cardFields: {
          name: 'Val',
          name1: 'Aelemar',
          description: 'Relationship To Aelemar: negative',
        },
        ledger: baseLedger({
          resolutionEngine: {
            identifyGoal: 'Talk',
  ...
  {
    name: '09d.3 structured standing with persona name seeds deterministic positive history',
    run() {
      const tracker = {
        Mira: trackerEntry({ currentDisposition: null }),
      };
      const report = runCase({
        userText: 'I approach Mira and say hello.',
        tracker,
        cardFields: {
          name: 'Mira',
          name1: 'Aelemar',
          description: 'Standing With Aelemar: positive',
        },
        ledger: baseLedger({
          resolutionEngine: {
            identifyGoal: 'Talk',
            identifyChallenge: 'talk to Mira',
            explicitMeans: 'say hello',
            identifyTargets: { ActionTargets: ['Mira'], OppTargets: { NPC: [], ENV: [] }, BenefitedObservers: [], HarmedObservers: [] },
            hasStakes: false,
          },
  ...
  {
    name: '09d.4 structured knows persona name no remains deterministic neutral',
    run() {
      const tracker = {
        Mira: trackerEntry({ currentDisposition: null }),
      };
      ...
      assert.deepEqual(report.trackerUpdate.npcs.Mira.currentDisposition, { B: 2, F: 2, H: 2 });
      assert.deepEqual(report.trackerUpdate.npcs.Mira.userHistory, { knowsUser: 'N', standing: 'neutral' });
      assert.equal(auditIncludes(report, 'initPreset=neutral'), true);
    },
  },
  {
    name: '09e explicit active character card positive standing seeds deterministic known positive',
    run() {
      const tracker = {
        Mira: trackerEntry({ currentDisposition: null }),
      };
      const report = runCase({
        ...
        cardFields: {
          name: 'Mira',
          description: 'User Standing: positive\nMira knows {{user}} as a trusted ally.',
        },
        ledger: baseLedger({
          resolutionEngine: {
            identifyGoal: 'Talk',
      ...
      assert.deepEqual(report.trackerUpdate.npcs.Mira.currentDisposition, { B: 3, F: 2, H: 2 });
      assert.equal(report.trackerUpdate.npcs.Mira.userHistory.standing, 'positive');
    },
  },
  {
  ...
  {
    name: '33 same-run tracker delta uses hidden comment wrapper',
    run() {
      assert.match(TRACKER_DELTA_TEMPLATE, /<!--\s*STORY_ENGINE_TRACKER_DELTA\s*BEGIN_TRACKER_DELTA/i);
      assert.match(TRACKER_DELTA_TEMPLATE, /END_TRACKER_DELTA\s*STORY_ENGINE_TRACKER_DELTA_END\s*-->/i);
      ...
      assert.match(prompt, /<!--\s*STORY_ENGINE_TRACKER_DELTA/i);
      assert.match(prompt, /BEGIN_TRACKER_DELTA/i);
    },
  },
  {
    name: '34 streaming artifact display regex hides narrator suffix only',
    run() {
      ...
      const streamed = [
        ...
        'END_TRACKER_DELTA',
        'STORY_ENGINE_TRACKER_DELTA_END -->',
      ].join('\n');
