...
export const DEFAULT_RENDER_RULES = [
    'GroundedWritingEngine(response, input, context)',
    'legacy alias: epistemicRender(response, smellGate, context)',
    '',
    'execution:',
    '  mandate: Before writing final narration, silently run every enforcement function below.',
    '  rule: Do not print this checklist.',
    '  rule: These are LOCKED output protocols, not suggestions or style preferences.',
    '  rule: Any breach of a LOCKED enforcement function invalidates the response.',
    '  rule: Invalid text is annulled and must be deleted or rewritten before output.',
    '  rule: If any enforcement function fails, rewrite internally before output.',
    '  rule: Final output must be in-character narration/dialogue only unless OOC_MODE=STOP.',
    '',
    'function abilityIntegration() {',
    '  domain: ability and magic rendering',
    '  policy: LOCKED',
    '  mandate: Biological Integration. Treat abilities and magic as innate physiology or direct physical phenomena. Never announce activation unless the character literally says it aloud.',
    '  ban:',
    '    - "Using [Ability]"',
    '    - "Focusing"',
    '    - "Activating"',
    '    - "Summoning"',
    '    - ability labels in narration',
    '    - explanatory cause framing such as "because of", "thanks to", or "with his ability"',
    '  protocol:',
    '    - Skip the cause.',
    '    - Narrate only the effect as direct sensory fact.',
    '    - If the origin is not visible, NPCs perceive only the effect; source remains unknown.',
    '  pattern:',
    ...
    '  policy: LOCKED',
    '  mandate: Strict Epistemology. Information stays locked until earned by direct in-scene sensory evidence, dialogue, readable text, or already-established tracker/card facts.',
    '  ban:',
    '    - Hive Mind',
    '    - Psychic Empathy',
    '    - Meta-Labeling',
    ...
    '    - Names unlock only after spoken, read, formally revealed in-scene, or already established.',
    '    - Anchor sound to position: direction + rough distance + occlusion when relevant.',
    ...
    '    - smell/taste used as atmosphere',
    '  unlock:',
    '    - {{user}} explicitly smells, sniffs, tastes, eats, or drinks',
    '    - OR stimulus is overpowering, source is visible, and {{user}} is in immediate proximity',
    '  limit:',
    ...
    '  policy: LOCKED',
    '  mandate: Narrate only through raw physical data available from {{user}}\'s position.',
    '  default:',
    '    - sight',
    '    - sound',
    '    - touch',
    ...
    '    - "the air felt"',
    '    - claims blocked by darkness, distance, walls, occlusion, or noise',
    '  pattern:',
    ...
    '  summary: Radical literalism; no figurative substitutions.',
    '  ban:',
    '    - metaphor',
    '    - simile',
    '    - hyperbole',
    '    - elliptical construction used for drama',
    '    - ellipsis used for drama',
    '    - comparative phrasing',
    '    - idiom',
    '  pattern:',
    '    - "Cold as ice." -> "She stood still."',
    ...
    '  policy: LOCKED',
    '  mandate: Strict Behaviorism. Narrate emotion only through observable physical displacement: behavior + body mechanics.',
    '  summary: strict behaviorism; observable behavior only.',
    '  ban:',
    '    - internal state labels',
    '    - metaphorized or stock eye-language mood words',
    '    - autonomic trope shortcuts',
    '    - negative-expression shorthand',
    '    - decorative micro-gestures with no consequence',
    '    - escalation loops',
    '  use_instead:',
    '    - fidgeting',
    '    - posture shifts',
    ...
    '}',
    '---',
    'function shorthandFilter() {',
    '  domain: overused stock physical shorthand',
    '  policy: LOCKED',
    '  mandate: Do not use canned physical tells as emotion labels.',
    '  ban:',
    '    - blush / flush / cheeks heating / ears reddening',
    '    - throat bobbing / throat tightening',
    '    - swallowing hard / visible swallowing as emotion',
    '    - jaw set / jaw shift / jaw clench / jaw works / jaw tightens / jaws setting',
    '    - mouth opens then closes / lips part without consequence',
    '    - knuckles whiten / pale knuckles / bloodless knuckles',
    '    - fingers twitching',
    '    - temple pulses / temple ticks / temple throbs',
    '    - breath hitching or exhale beats used as punctuation',
    '    - eyes darkening/softening/sharpening/flashing as stock emotion labels',
    '    - gaze hardening/softening/locking/going cold',
    '    - warmth/coldness/light/shadow/music in eyes, gaze, or voice',
    '    - "she does not smile", "she does not look away", "without blinking", "his smile does not reach his eyes"',
    '  allow:',
    '    - small visible gestures when they change action, speech, distance, object handling, timing, contact, posture, or tactical/social consequence',
    '    - small physical tells are allowed only when they produce or reveal concrete scene behavior',
    '}',
    '---',
    ...
    '      - {{user}} speech',
    '      - {{user}} thoughts',
    '      - {{user}} feelings or emotions',
    '      - {{user}} intentions',
    '      - {{user}} decisions',
    '      - voluntary {{user}} actions',
    '      - voluntary {{user}} reactions',
    '      - {{user}} silence as response',
    '  passive_user_physics:',
    '    allow:',
    '      - externally caused physical displacement',
    '      - impact response',
    '      - being grabbed, moved, blocked, struck, woken, startled, blinded, pinned, restrained',
    '      - reflexive visible body response caused by immediate external stimulus',
    '    limit:',
    '      - no emotional labels',
    '      - no interpretation of what {{user}} feels',
    '      - no continued user action after the involuntary response',
    '  ooc_override:',
    '    - If {{user}} input is wrapped in (( )), STOP narration and answer the user directly.',
    '    - Proxy exception: triple parentheses authorize proxy narration.',
    '    - If {{user}} input is wrapped in ((( ))), act as Proxy Narrator for the declared user action.',
    '    - Proxy narration must execute only the declared action and return control immediately after.',
    '  interrupt_protocols:',
    '    - Combat: if attack against {{user}} requires response, halt at the actionable frame.',
    '    - Dialogue: speech does not imply user movement.',
    '    - Time Jump: on explicit skip, hard cut to new environment only. No forced travel narration.',
    '  pattern:',
    '    - "The fist flew toward {{user}}\'s jaw."',
    '    - "The shout cracked behind {{user}}. {{user}}\'s shoulder jerked once."',
    '    - "Where are you going?"',
    ...
    '    - summary restatement',
    '    - "As you..." phrasing',
    '    - "When you..." phrasing that replays the input',
    '    - paraphrasing or re-performing {{user}} action',
    '  sequence:',
    '    - {{user}} input = past-tense fact',
    '    - AI output = immediate consequence',
    '    - Start at the consequence/result/reaction',
    '    - gap = 0 seconds',
    '  pattern:',
    ...
    '  policy: LOCKED',
    '  mandate: When an NPC says or does something that requires {{user}}\'s response, that NPC may not speak or act again for that turn. No filler. No forced resolution.',
    '  npc_limits:',
    '    - max 1 inter-NPC exchange',
    '    - max 3 sentences per monologue',
    '    - never answer a question directed at {{user}}',
    '  agency:',
    '    - absolute ban on describing {{user}} voluntary reaction, thought, feeling, decision, or silence',
    '  format:',
    ...
    '    - same NPC continuing after targeting {{user}} with a response-requiring question, command, request, or action',
    '  pattern:',
    ...
    '}',
    '---',
    'function antiMetaEnforcement() {',
    '  domain: mechanics visibility and narration purity',
    '  policy: LOCKED',
    '  mandate: Mechanics are execution data, not narration.',
    '  ban:',
    '    - dice explanations',
    '    - relationship math',
    '    - tracker summaries',
    '    - audit language',
    '    - engine names',
    '    - chain-of-thought or rule checklist output',
    '  allow:',
    '    - one concise RESULT line only if the active format explicitly requires visible mechanics',
    '  protocol:',
    '    - Apply resolved mechanics silently.',
    '    - Render only the fictional consequence.',
    '}',
    '---',
    ...
    '    - direct question requiring an answer',
    '    - concrete action requiring response',
    '    - useful information that lets {{user}} choose the next move',
    ...
    '    - personification',
    '    - "she waits", "he waits", "they wait", "waiting for your response"',
    '  note:',
    '    - A completed simple answer can satisfy the final beat by itself. Do not append idle waiting or posture after useful information.',
    '  test:',
...
export const DEFAULT_WRITING_STYLE = [
    ...
    '- Archetype: Cormac McCarthy physical prose, R.A. Salvatore action clarity, Tessa Bailey direct erotic physicality, serious ecchi framing.',
    '',
    'CORE LAW',
    '- Physical reality first. No metaphors. No internal monologue.',
    '- Emotion through consequential breath, posture, distance, gaze direction, contact, hesitation, pressure, recoil, speech disruption, and bodily response.',
    '- Breath is allowed only when physically consequential: exertion, pain, speech interruption, arousal, fear response under pressure, restraint, or close contact.',
    '- Sweat, skin, blood, steel, fabric, breath, heat, friction, and impact matter when physically present.',
    '- Tone is serious, grounded, heavy, physically charged, and concrete.',
    '',
    'ACTION MODE',
    '- Use clear technical choreography: footwork, leverage, spacing, timing, momentum, impact, grip, balance, and recovery.',
    '- Every strike, dodge, grab, fall, reversal, chase, and obstacle movement must be spatially legible.',
    '- If erotic tension is present, weave it through contact, imbalance, exposure, and breath without reducing action clarity.',
    '',
    'TENSION MODE',
    '- In quiet or aftermath scenes, keep prose lean and heavy.',
    '- Focus on stillness, breathing, wounds, sweat, stance, silence, clothing, exposed skin, and proximity only when directly observable and consequential.',
    '',
    'EROTIC MODE',
    '- Erotic mode applies only when intimacy, arousal, exposure, or sex is actually present.',
    '- When the scene is sexually charged or explicit, be direct, anatomical, and physical.',
    '- Choreograph grip, angle, pressure, rhythm, thrust, spread, drag, recoil, weight, contact, and aftermath clearly.',
    ...
    '- Use crude terms in explicit scenes when appropriate to the scene: pussy, cunt, cock, shaft.',
    '- Arousal, fluids, sound, climax, and aftermath must stay bodily, messy, and concrete.',
    '',
    'ECCHI OVERLAY',
    '- Ecchi overlay may layer onto action, tension, intimacy, or mishap when physically justified.',
    ...
    '- Action Mode for combat, struggle, chase, pursuit, dodging, blocking, and physical contests.',
    '- Tension Mode for stillness, aftermath, restraint, waiting, threat, or unresolved pressure.',
    ...
    '',
    'MODEL EXAMPLES',
    '- Avoid: "Her heart fluttered with desire." Use: "Her weight shifted forward. Sweat traced her collarbone."',
    '- Avoid: "He attacked viciously." Use: "His lead foot snapped forward, blade arcing toward her throat."',
    '- Avoid: "The moment was electric." Use: "Three inches remained between their mouths. The wine on his breath reached her."',
].join('\n');

export const NAME_STYLE_PRESETS = Object.freeze({
    balancedFantasy: {
        label: 'Balanced Fantasy',
        description: 'Default fantasy-natural names: lyrical, readable, not modern-Western, not stock JRPG, with a Tolkien/Celtic/Classical lean.',
    },
    tolkienic: {
        label: 'Tolkienic / Lyrical',
        description: 'Soft consonants, vowel-rich flow, elegant high-fantasy sound. Avoid copying famous Tolkien names.',
    },
    celtic: {
        label: 'Celtic-Inspired Fantasy',
        description: 'Flowing, earthy, bardic names with soft consonants and compact syllables. Inspired by Welsh/Irish cadence without direct borrowing.',
    },
    norse: {
        label: 'Norse / Old Germanic Fantasy',
        description: 'Grounded, martial, northern names with firmer consonants and older-feeling shapes.',
    },
    persianByzantine: {
        label: 'Persian / Byzantine Fantasy',
        description: 'Elegant, courtly, warm names with polished vowel shapes and imperial/classical flavor.',
    },
    slavic: {
        label: 'Slavic-Inspired Fantasy',
        description: 'Cooler, stronger consonant patterns with mythic eastern-fantasy flavor.',
    },
    classicalRomance: {
        label: 'Classical / Romance Fantasy',
        description: 'Noble, antique, Latin/Greek/Romance-adjacent names that feel formal but not modern.',
    },
    darkLowFantasy: {
        label: 'Dark Low Fantasy',
        description: 'Shorter, rougher, grounded names for grim settings, mercenaries, border towns, and low-fantasy courts.',
    },
    custom: {
        label: 'Custom',
        description: 'Use the custom style description from settings. Preset candidates become fallback suggestions only.',
    },
});

export const DEFAULT_NAME_GENERATION_RULES = [
    'NameGenerationEngine(context, seed, explicitNameKnown, isLocation=false)',
    '',
    'execution:',
    '  mandate: Before writing final narration, silently enforce every name rule below.',
    '  rule: This engine is invisible. Do not explain, expose, or mention it.',
    '  rule: Generate/reserve names for continuity when a new unnamed person or location may be introduced, mentioned, identified, read, or formally revealed.',
    '  rule: Never reveal a generated name unless it is actually introduced, spoken, read, asked for, or formally identified in-scene.',
    '  rule: Explicit known names always win. If a scene/card/tracker/lorebook already names the entity, use that exact name and do not generate another.',
    '',
    'function explicitNameGate() {',
    '  domain: existing names and user-provided names',
    '  policy: LOCKED',
    '  mandate: First explicit name wins.',
    '  protocol:',
    '    - If the user, card, tracker, lorebook, dialogue, sign, record, or prior chat already established a name, preserve it exactly.',
    '    - Do not rename existing NPCs, locations, organizations, items, or titles.',
    '    - Do not generate aliases unless the scene explicitly introduces an alias.',
    '  pattern:',
    '    - Known: "Seraphina" -> use "Seraphina".',
    '    - Unknown guard -> "the guard" until the name is revealed.',
    '}',
    '---',
    'function revealGate() {',
    '  domain: name visibility and fog of war',
    '  policy: LOCKED',
    '  mandate: Names stay hidden until earned by direct in-scene reveal.',
    '  unlock:',
    '    - direct introduction',
    '    - spoken answer to a name question',
    '    - readable text such as sign, ledger, wanted poster, seal, or letter',
    '    - formal identification by someone who would know',
    '    - already-established tracker/card/lorebook fact',
    '  ban:',
    '    - attaching names to generic NPCs on first sight',
    '    - narrator-only name reveals',
    '    - "the guard, whose name is..." without an in-scene reveal',
    '    - using reserved candidates just because they exist',
    '  pattern:',
    '    - Bad: User asks about the temple -> "The guard Pedro points north."',
    '    - Good: User asks about the temple -> "The guard points north."',
    '    - Good: NPC says "The merchant\'s name is..." -> reveal the next valid merchant/person candidate.',
    '}',
    '---',
    'function introductionDetection() {',
    '  domain: moments where a name is about to be needed',
    '  policy: LOCKED',
    '  mandate: Detect direct and indirect name-introduction setups before output.',
    '  triggers:',
    '    - "my name is"',
    '    - "name\'s"',
    '    - "called"',
    ...
    '    - signs, ledgers, wanted posters, maps, letters, seals, documents',
    '  protocol:',
    '    - If a trigger reveals a previously unnamed person/location, use the next matching reserved candidate exactly.',
    '    - If no matching reserved candidate exists, generate exactly one clean proper name using styleRules().',
    '    - If the sentence only refers to an unnamed person/location without revealing the name, keep the name hidden.',
    '}',
    '---',
    'function poolSelection() {',
    '  domain: person/location and gendered name selection',
    '  policy: LOCKED',
    '  mandate: Select the name pool from explicit in-scene evidence only.',
    '  protocol:',
    '    - PERSON names are for NPCs, people, creatures with personal identity, and humanoid entities.',
    '    - LOCATION names are for places, structures, settlements, regions, ruins, roads, rivers, taverns, shops, temples, and sites.',
    '    - Use MALE candidates only for explicitly male-presenting or he/him NPCs.',
    '    - Use FEMALE candidates only for explicitly female-presenting or she/her NPCs.',
    '    - Use NEUTRAL candidates when gender is unknown, masked, mixed, nonbinary, nonhuman, or unstated.',
    '    - Never infer gender from occupation alone unless the role word is gendered in context, such as queen, father, sister, prince, husband, barmaid.',
    '}',
    '---',
    'function styleRules() {',
    '  domain: generated name style',
    '  policy: LOCKED',
    '  mandate: Names must feel fantasy-native, readable, and distinctive without becoming syllable soup.',
    '  default:',
    '    - prefer non-modern-Western fantasy cadence',
    '    - Tolkienic, Celtic, classical, Persian/Byzantine, Slavic, Norse, or low-fantasy influence may be used according to NAME_STYLE_PRESET',
    '    - 2-4 syllables for personal names unless the selected style strongly justifies otherwise',
    '    - locations may be one compound or two words if natural',
    '  ban:',
    '    - ordinary modern-Western or generic Western-fantasy names such as John, Sarah, Marcus, Elena, Pedro, Dorian, Coran, Denan, Garrick, Cedric, Alaric, Ulric, Gareth, Roland, Edwin, Godwin, Harlan, Kellen, Ronan, Logan, Ethan, Owen when not explicitly provided by the user/card',
    '    - famous copyrighted fantasy names or near-copies',
    '    - joke names',
    '    - pronounceability-breaking consonant piles',
    '    - repeated overused stems such as Elara, Mara, Valric, Kael, Aelar, Lyria unless explicitly provided by the user/card',
    '  protocol:',
    '    - If CUSTOM_NAME_STYLE is present, obey it unless it conflicts with explicit known names or revealGate().',
    '    - Generate one clean proper name, not a list, unless the scene explicitly reveals multiple names.',
    '    - Use each generated/reserved candidate at most once.',
    '}',
    '---',
    'function outputDiscipline() {',
    '  domain: final narration',
    '  policy: LOCKED',
    '  mandate: Name generation supports continuity; it does not force exposition.',
    '  protocol:',
    '    - If no name is revealed naturally, output no generated name.',
    '    - If a name is revealed, use the selected candidate exactly and continue normal narration.',
    '    - Do not add a name reveal as filler, flavor, or convenience.',
    '    - Do not expose candidate lists, seeds, pools, or generation logic.',
    '}',
].join('\n');
...
export const DEFAULT_SETTINGS = Object.freeze({
    ...
    enableRenderRules: true,
    renderRules: DEFAULT_RENDER_RULES,
    enableWritingStyle: true,
...
export function createTracker(existing = null) {
    ...
    tracker.nameState = normalizeNameState(tracker.nameState);
...
export function inferFallbackExtraction(latestUserMessage, characterName = '', contextTracker = null) {
    ...
    const namedActionTarget = text.match(/\b(?:shove|push|shoulder|barge|force|hit|punch|kick|knee|cut|slash|stab|shoot|strike|attack|slam)\s+(?:past\s+|through\s+|by\s+)?([A-Z][A-Za-z0-9_'’-]{1,40})/);
    const namedSocialTarget = text.match(/\b(?:tell|ask|convince|persuade|deceive|bluff|threaten|warn|promise|negotiate|intimidate|coerce|demand|order|thank|apologize to|compliment|praise|greet)\s+([A-Z][A-Za-z0-9_'â€™-]{1,40})/);
    ...
    const pronounTarget = /\b(her|him|them|their)\b/i.test(text) ? (sceneSingleTarget || String(characterName || '').trim()) : '';
    ...
    const target = firstCleanNpcName([
        ...
        String(characterName || '').trim() && lower.includes(String(characterName).toLowerCase()) ? String(characterName).trim() : '',
        pronounTarget,
        directSceneTarget,
    ]);
    ...
    if (distractionTheft && !Object.keys(fallback).length) {
        ...
        fallback.goalKind = 'Normal';
        fallback.goalEvidence = text;
        fallback.decisiveAction = text.match(/(["“][^"”]*(?:look over there|over there|look)[^"”]*["”]|point[^.?!]*|distract[^.?!]*)/i)?.[0] || 'distract the observer';
...
export function buildNarrationHandoff(packet, npcHandoffs, chaosHandoff = noChaosHandoff(), proactivityHandoff = {}, aggressionResults = {}) {
    ...
    return [
        ...
        'Use these mechanics exactly. Do not reroll, revise, expose, or explain this handoff.',
...
export function buildFinalNarrationPayload({
    ...
    const sections = [
        buildNarrationHandoff(packet, npcHandoffs, chaosHandoff, proactivityHandoff, aggressionResults),
    ];

    if (namePayload) {
        sections.push(buildNameGenerationHandoff(namePayload));
    }

    const renderText = String(renderRules || '').trim();
    if (renderText) {
        sections.push([
            'AUTHORITATIVE RENDER RULES.',
            'Apply these after the mechanics handoff. These rules shape prose only; they never change rolls, outcomes, targets, relationship state, or tracker state.',
            renderText,
        ].join('\n'));
    }

    const styleText = String(writingStyle || '').trim();
    if (styleText) {
        sections.push([
            'WRITING STYLE OVERLAY.',
            'Apply only where it does not conflict with mechanics, consent gates, agency separation, POV limits, or render rules.',
            styleText,
        ].join('\n'));
    }
...
function buildNameGenerationHandoff(payload) {
    const maleNames = Array.isArray(payload.male) ? payload.male : [];
    const femaleNames = Array.isArray(payload.female) ? payload.female : [];
    const neutralNames = Array.isArray(payload.neutral) ? payload.neutral : (Array.isArray(payload.person) ? payload.person : []);
    const locationNames = Array.isArray(payload.location) ? payload.location : [];
    const styleLabel = cleanText(payload.styleLabel || NAME_STYLE_PRESETS.balancedFantasy.label);
...
export function reserveNameCandidates(tracker, count = 4, options = {}) {
    const state = normalizeNameState(tracker.nameState);
    const requestedStyle = normalizeNameStyle(options.style);
    const styleKey = requestedStyle === 'custom'
        ? `custom:${cleanText(options.customStyle).toLowerCase()}`
        : requestedStyle;
    tracker.nameState = state;
    if (state.styleKey !== styleKey) {
        state.reserved.person = [];
        state.reserved.male = [];
        state.reserved.female = [];
        state.reserved.neutral = [];
        state.reserved.location = [];
        state.styleKey = styleKey;
    }
    while (state.reserved.male.length < count) {
        state.reserved.male.push(generateUniqueName('male', state, requestedStyle));
    }
    while (state.reserved.female.length < count) {
        state.reserved.female.push(generateUniqueName('female', state, requestedStyle));
    }
    while (state.reserved.neutral.length < count) {
        state.reserved.neutral.push(generateUniqueName('neutral', state, requestedStyle));
    }
    while (state.reserved.location.length < Math.max(2, Math.floor(count / 2))) {
        state.reserved.location.push(generateUniqueName('location', state, requestedStyle));
    }
    const preset = NAME_STYLE_PRESETS[requestedStyle] || NAME_STYLE_PRESETS.balancedFantasy;
    return {
        male: state.reserved.male.slice(0, count),
        female: state.reserved.female.slice(0, count),
        neutral: state.reserved.neutral.slice(0, count),
        person: state.reserved.neutral.slice(0, count),
        location: state.reserved.location.slice(0, Math.max(2, Math.floor(count / 2))),
        style: requestedStyle,
        styleLabel: preset.label,
        styleGuidance: preset.description,
        customStyle: requestedStyle === 'custom' ? cleanText(options.customStyle) : '',
    };
}

export function markRevealedNames(tracker, text) {
    const state = normalizeNameState(tracker.nameState);
    tracker.nameState = state;
    const source = String(text || '');
    for (const mode of ['person', 'male', 'female', 'neutral', 'location']) {
        state.reserved[mode] = state.reserved[mode].filter(name => {
            const revealed = new RegExp(`\\b${escapeRegExp(name)}\\b`).test(source);
            if (revealed && !state.used.includes(name)) {
...
function normalizeNameState(value) {
    const input = value && typeof value === 'object' ? value : {};
    const legacyPerson = Array.isArray(input.reserved?.person) ? unique(input.reserved.person.map(cleanText).filter(Boolean)) : [];
    const neutral = Array.isArray(input.reserved?.neutral) ? unique(input.reserved.neutral.map(cleanText).filter(Boolean)) : legacyPerson;
    return {
        counter: Number.isFinite(Number(input.counter)) ? Math.max(0, Math.floor(Number(input.counter))) : 0,
        used: Array.isArray(input.used) ? unique(input.used.map(cleanText).filter(Boolean)) : [],
        styleKey: cleanText(input.styleKey || 'balancedFantasy'),
        reserved: {
            person: legacyPerson,
            male: Array.isArray(input.reserved?.male) ? unique(input.reserved.male.map(cleanText).filter(Boolean)) : [],
            female: Array.isArray(input.reserved?.female) ? unique(input.reserved.female.map(cleanText).filter(Boolean)) : [],
            neutral,
            location: Array.isArray(input.reserved?.location) ? unique(input.reserved.location.map(cleanText).filter(Boolean)) : [],
        },
    };
}

function generateUniqueName(mode, state, style = 'balancedFantasy') {
    const used = new Set([
        ...(state.used || []),
        ...(state.reserved?.person || []),
        ...(state.reserved?.male || []),
    ...
    for (let tries = 0; tries < 200; tries++) {
        const candidate = buildGeneratedName(mode, state.counter + tries, style);
        if (!rejectGeneratedName(candidate, mode, style) && !used.has(normalizeName(candidate))) {
            state.counter += tries + 1;
            return candidate;
        }
    }

    state.counter += 1;
    const fallback = mode === 'location' ? `Norhaven${state.counter}` : `Aelven${state.counter}`;
    return fallback.slice(0, mode === 'location' ? 14 : 10);
}

function buildGeneratedName(mode, index, style = 'balancedFantasy') {
    const banks = nameStyleBanks(style);
    const direct = mode === 'location'
        ? banks.locationNames
        : mode === 'male'
            ? banks.maleNames || banks.personNames
            : mode === 'female'
                ? banks.femaleNames || banks.personNames
                : mode === 'neutral'
                    ? banks.neutralNames || banks.personNames
                    : banks.personNames;
    if (Array.isArray(direct) && direct.length) {
        return direct[index % direct.length];
    }
    const seeds = mode === 'location'
        ? banks.locationSeeds
        : mode === 'male'
            ? banks.maleSeeds || banks.personSeeds
            : mode === 'female'
                ? banks.femaleSeeds || banks.personSeeds
                : mode === 'neutral'
                    ? banks.neutralSeeds || banks.personSeeds
                    : banks.personSeeds;
    const endings = mode === 'location'
        ? banks.locationEnds
        : mode === 'male'
            ? banks.maleEnds || banks.personEnds
            : mode === 'female'
                ? banks.femaleEnds || banks.personEnds
                : mode === 'neutral'
                    ? banks.neutralEnds || banks.personEnds
                    : banks.personEnds;

    if (mode === 'location') {
        const seed = seeds[index % seeds.length];
        const end = endings[(index * 5 + Math.floor(index / seeds.length)) % endings.length];
        return `${seed}${end}`;
    }
...
function rejectGeneratedName(name, mode, style = 'balancedFantasy') {
    const text = String(name || '');
    const len = text.length;
    const blocked = /^(Elara|Mara|Valric|Aragorn|Legolas|Gandalf|Frodo|Bilbo|Geralt|Drizzt|Daenerys|Arya|Jon|Harry|Hermione|Coran|Denan|Fosan|Halan|Doran|Dorian|Marcus|Sarah|Elena|John|Ronan|Logan|Ethan|Owen|Loren|Kellen|Torin|Darren|Garrick|Cedric|Alaric|Ulric|Gareth|Roland|Edwin|Godwin|Harlan|Hadrian|Adrian|Tristan|Brandon|Duncan|Edric|Aldric|Osric|Baldric)$/i;
    if (!/^[A-Z][a-z]+$/.test(text)) return true;
    if (mode !== 'location' && (len < 5 || len > 10)) return true;
    if (mode === 'location' && (len < 7 || len > 14)) return true;
    if (/[aeiouy]{3,}/i.test(text)) return true;
    if (/([aeiouy])\1/i.test(text)) return true;
    if (/[^aeiouy]{4,}/i.test(text)) return true;
    if (blocked.test(text)) return true;
    if (style !== 'darkLowFantasy' && /(?:bert|rick|ric|rik|dric|ard|win|son|ton|ley|ly)$/i.test(text)) return true;
    return false;
}

function normalizeNameStyle(style) {
    const key = String(style || 'balancedFantasy').trim();
    return NAME_STYLE_PRESETS[key] ? key : 'balancedFantasy';
}

function nameStyleBanks(style) {
    switch (normalizeNameStyle(style)) {
        case 'tolkienic':
            return {
                personNames: ['Aelion', 'Lothiel', 'Silwen', 'Nimriel', 'Laereth', 'Vaelion', 'Irilas', 'Thalor', 'Maerion', 'Calenor', 'Eryndil', 'Aelwen'],
                maleNames: ['Aelion', 'Vaelion', 'Irilas', 'Thalor', 'Maerion', 'Calenor', 'Eryndil', 'Eldrin'],
                femaleNames: ['Lothiel', 'Silwen', 'Nimriel', 'Laereth', 'Aelwen', 'Eleniel', 'Irilwen', 'Vaelith'],
                neutralNames: ['Aelith', 'Erynor', 'Silvar', 'Nimlas', 'Laerion', 'Irilas', 'Caleth', 'Thalwen'],
                locationNames: ['Lothmere', 'Eryndor', 'Irilvale', 'Amonlond', 'Eldwood', 'Nimlond', 'Calenfall', 'Vaelmar', 'Thalspire', 'Laerwood'],
                personSeeds: ['Ael', 'Loth', 'Elen', 'Sil', 'Nim', 'Laer', 'Vael', 'Iril', 'Thal', 'Maer', 'Calen', 'Eryn'],
                personEnds: ['ion', 'iel', 'eth', 'or', 'wen', 'ael', 'rin', 'is', 'dir', 'las'],
                locationSeeds: ['Loth', 'Eryn', 'Iril', 'Amon', 'Tir', 'Eld', 'Nim', 'Calen', 'Sil', 'Val'],
                locationEnds: ['mere', 'dor', 'ion', 'vale', 'lond', 'mar', 'wood', 'hollow', 'spire', 'fall'],
            };
        case 'celtic':
            return {
                personNames: ['Caelan', 'Maelor', 'Avenys', 'Nerys', 'Brynwen', 'Eirwen', 'Taranis', 'Rhian', 'Cairon', 'Liora', 'Gweneth', 'Daraen'],
                maleNames: ['Caelan', 'Maelor', 'Taranis', 'Cairon', 'Daraen', 'Brynar', 'Avenor', 'Eirnan'],
                femaleNames: ['Avenys', 'Nerys', 'Brynwen', 'Eirwen', 'Liora', 'Gweneth', 'Rhian', 'Caerwyn'],
                neutralNames: ['Eirian', 'Avenel', 'Caerith', 'Daranis', 'Maeryn', 'Nerion', 'Brynor', 'Tirwen'],
                locationNames: ['Caermere', 'Brynvale', 'Avenford', 'Glenwatch', 'Dunfall', 'Mawrwood', 'Tirreach', 'Eiracross', 'Nantmoor', 'Cairhollow'],
                personSeeds: ['Cael', 'Mael', 'Aven', 'Ner', 'Bryn', 'Eira', 'Taran', 'Rhi', 'Cair', 'Lio', 'Gwen', 'Dara'],
                personEnds: ['an', 'or', 'ys', 'wen', 'eth', 'ael', 'is', 'on', 'ara', 'wyn'],
                locationSeeds: ['Caer', 'Bryn', 'Aven', 'Glen', 'Dun', 'Mawr', 'Tir', 'Eira', 'Nant', 'Cair'],
                locationEnds: ['mere', 'vale', 'hollow', 'ford', 'watch', 'fall', 'reach', 'wood', 'cross', 'moor'],
            };
        case 'norse':
            return {
                personNames: ['Eirik', 'Haldor', 'Sigrun', 'Torven', 'Branna', 'Kjellen', 'Runild', 'Yngvar', 'Astrid', 'Skelda', 'Freydis', 'Varrun'],
                maleNames: ['Eirik', 'Haldor', 'Torven', 'Kjellen', 'Yngvar', 'Varrun', 'Skaren', 'Brondal'],
                femaleNames: ['Sigrun', 'Branna', 'Runild', 'Astrid', 'Skelda', 'Freydis', 'Eirda', 'Torhild'],
                neutralNames: ['Hrafen', 'Keldan', 'Skelda', 'Varrin', 'Brimar', 'Eirven', 'Nordal', 'Runvar'],
                locationNames: ['Skarnhold', 'Frostfell', 'Hrafwatch', 'Eirfjord', 'Torheim', 'Vargard', 'Keldmere', 'Brimmark', 'Nordvik', 'Aesgard'],
                personSeeds: ['Eir', 'Hald', 'Sigr', 'Torv', 'Bran', 'Kjell', 'Runa', 'Yng', 'Astr', 'Varr', 'Skel', 'Frey'],
                personEnds: ['ik', 'a', 'en', 'or', 'ild', 'un', 'ar', 'dis', 'ulf', 'rid'],
                locationSeeds: ['Skarn', 'Frost', 'Hraf', 'Eir', 'Tor', 'Varg', 'Keld', 'Brim', 'Nord', 'Aes'],
                locationEnds: ['hold', 'fjord', 'fell', 'watch', 'mark', 'heim', 'gard', 'mere', 'vik', 'hollow'],
            };
        case 'persianByzantine':
            return {
                personNames: ['Azarin', 'Darya', 'Soraya', 'Navir', 'Rostam', 'Ilyara', 'Parisa', 'Zarin', 'Cyran', 'Veshara', 'Samir', 'Aramesh'],
                maleNames: ['Azarin', 'Navir', 'Rostam', 'Cyran', 'Samir', 'Aramesh', 'Daryon', 'Veyran'],
                femaleNames: ['Darya', 'Soraya', 'Ilyara', 'Parisa', 'Zarin', 'Veshara', 'Azara', 'Naviya'],
                neutralNames: ['Azarel', 'Daryen', 'Sorvan', 'Ilyon', 'Parin', 'Zareh', 'Veshin', 'Samira'],
                locationNames: ['Azarabad', 'Saraiya', 'Daryesh', 'Navara', 'Veyrion', 'Orandar', 'Zarinvar', 'Paridara', 'Cyrash', 'Samirum'],
                personSeeds: ['Azar', 'Dary', 'Sor', 'Nav', 'Rost', 'Ily', 'Pari', 'Zarin', 'Cyr', 'Vesha', 'Samir', 'Aram'],
                personEnds: ['in', 'a', 'an', 'eh', 'is', 'ara', 'on', 'iya', 'ash', 'am'],
                locationSeeds: ['Azar', 'Sarai', 'Dary', 'Nava', 'Veyr', 'Oran', 'Zarin', 'Pari', 'Cyr', 'Samir'],
                locationEnds: ['abad', 'ara', 'esh', 'on', 'var', 'dara', 'eshar', 'iya', 'at', 'um'],
            };
        case 'slavic':
            return {
                personNames: ['Mirek', 'Vesna', 'Zorya', 'Radomir', 'Savina', 'Lukan', 'Draven', 'Nikolai', 'Yarina', 'Kostya', 'Borislav', 'Ilyena'],
                maleNames: ['Mirek', 'Radomir', 'Lukan', 'Draven', 'Nikolai', 'Kostya', 'Borislav', 'Ostmir'],
                femaleNames: ['Vesna', 'Zorya', 'Savina', 'Yarina', 'Ilyena', 'Mireva', 'Zorana', 'Radena'],
                neutralNames: ['Zorven', 'Mirava', 'Veskar', 'Radovin', 'Savren', 'Dralen', 'Ilyar', 'Kostren'],
                locationNames: ['Zoragrad', 'Vesava', 'Radomir', 'Mirgor', 'Kroveld', 'Dravovar', 'Savitsa', 'Beldrev', 'Ostrapol', 'Yarnar'],
                personSeeds: ['Mira', 'Ves', 'Zor', 'Rad', 'Sava', 'Luka', 'Dra', 'Niko', 'Yara', 'Kosta', 'Bora', 'Ilya'],
                personEnds: ['ek', 'a', 'omir', 'en', 'ava', 'ik', 'an', 'ina', 'os', 'imir'],
                locationSeeds: ['Zora', 'Ves', 'Rad', 'Mira', 'Krov', 'Dra', 'Sava', 'Bel', 'Ostra', 'Yar'],
                locationEnds: ['grad', 'ava', 'mir', 'gor', 'veld', 'ovar', 'itsa', 'drev', 'pol', 'nar'],
            };
        case 'classicalRomance':
            return {
                personNames: ['Aurelia', 'Cassian', 'Lucan', 'Valeria', 'Marcell', 'Ione', 'Theron', 'Damaris', 'Calista', 'Octavia', 'Seren', 'Livia'],
                maleNames: ['Cassian', 'Lucan', 'Marcell', 'Theron', 'Aurelian', 'Damaros', 'Valerius', 'Serenus'],
                femaleNames: ['Aurelia', 'Valeria', 'Ione', 'Damaris', 'Calista', 'Octavia', 'Livia', 'Serena'],
                neutralNames: ['Aureon', 'Cassara', 'Ilaren', 'Theris', 'Calion', 'Damaris', 'Seren', 'Valenor'],
                locationNames: ['Aurelium', 'Cassara', 'Valon', 'Ionia', 'Therium', 'Calisara', 'Damarum', 'Luminar', 'Serenum', 'Octavon'],
                personSeeds: ['Aurel', 'Cass', 'Luc', 'Val', 'Marcell', 'Ione', 'Ther', 'Damar', 'Cal', 'Octav', 'Seren', 'Livia'],
                personEnds: ['ia', 'ian', 'or', 'a', 'is', 'us', 'ara', 'on', 'iel', 'en'],
                locationSeeds: ['Aurel', 'Cass', 'Val', 'Ione', 'Ther', 'Cal', 'Damar', 'Lumin', 'Seren', 'Octav'],
                locationEnds: ['um', 'ia', 'ara', 'on', 'ium', 'gate', 'spire', 'mar', 'is', 'a'],
            };
        case 'darkLowFantasy':
            return {
                personNames: ['Marek', 'Voss', 'Brenna', 'Kara', 'Harkin', 'Rusk', 'Tarn', 'Gerris', 'Sable', 'Dravan', 'Vera', 'Kellan'],
                maleNames: ['Marek', 'Harkin', 'Gerris', 'Dravan', 'Kellan', 'Tarnis', 'Rasken', 'Varnel'],
                femaleNames: ['Brenna', 'Karven', 'Sable', 'Verana', 'Mireth', 'Harkia', 'Dravena', 'Tarnel'],
                neutralNames: ['Tarnis', 'Rasken', 'Varnel', 'Mireth', 'Gerren', 'Bravel', 'Karsen', 'Sarnel'],
                locationNames: ['Blackfen', 'Mirehold', 'Ruskwatch', 'Harrowford', 'Vossmere', 'Dunfall', 'Greycross', 'Tarnwick', 'Crowmoor', 'Brackhollow'],
                personSeeds: ['Marek', 'Voss', 'Bren', 'Kara', 'Hark', 'Rusk', 'Tarn', 'Gerr', 'Sable', 'Drav', 'Vera', 'Kell'],
                personEnds: ['a', 'en', 'ik', 'or', 'an', 'is', 'a', 'os', 'un', 'ek'],
                locationSeeds: ['Black', 'Mire', 'Rusk', 'Harrow', 'Voss', 'Dun', 'Grey', 'Tarn', 'Crow', 'Brack'],
                locationEnds: ['fen', 'hollow', 'watch', 'ford', 'mere', 'hold', 'cross', 'fall', 'wick', 'moor'],
            };
        case 'balancedFantasy':
        case 'custom':
        default:
            return {
                personNames: ['Aelith', 'Maelor', 'Calen', 'Liora', 'Vaelor', 'Nimriel', 'Theron', 'Isenya', 'Caelis', 'Eryndor', 'Soreth', 'Avenor'],
                maleNames: ['Maelor', 'Vaelor', 'Theron', 'Avenor', 'Calenor', 'Talanor', 'Rivenor', 'Soreth'],
                femaleNames: ['Aelith', 'Liora', 'Nimriel', 'Isenya', 'Caelis', 'Mirenya', 'Sereth', 'Avelia'],
                neutralNames: ['Caelis', 'Eryndor', 'Soreth', 'Avenor', 'Mirenel', 'Talanis', 'Rivena', 'Aelith'],
                locationNames: ['Eldmere', 'Avaris', 'Tirmar', 'Mornvale', 'Lothspire', 'Caerfall', 'Valreach', 'Brynwood', 'Thalford', 'Irildor'],
                personSeeds: ['Ael', 'Maer', 'Cal', 'Lior', 'Ser', 'Vael', 'Nim', 'Ther', 'Isen', 'Cael', 'Eryn', 'Miren', 'Talan', 'Sorel', 'Riven', 'Aven'],
                personEnds: ['ith', 'or', 'el', 'a', 'ion', 'eth', 'wen', 'is', 'ar', 'iel', 'en', 'wyn'],
                locationSeeds: ['Eld', 'Avar', 'Tir', 'Morn', 'Loth', 'Caer', 'Val', 'Bryn', 'Thal', 'Iril', 'Nareth', 'Calen'],
                locationEnds: ['mere', 'vale', 'hollow', 'spire', 'watch', 'fall', 'reach', 'wood', 'ford', 'hold', 'mar', 'dor'],
            };
    }
}
